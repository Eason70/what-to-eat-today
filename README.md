...
经站点所有者确认可消耗高德额度及承担账户规则下的调用费用，已启用定位、地点搜索和附近查店。三个高德凭据仍只保存为后台 Worker Secret，未进入公开源码或 Pages 资源；没有购买套餐或充值。所有访客使用站点所有者的高德额度。Pages 的真实地点搜索、附近查店、逆地理编码及 SDK 坐标转换均已获得成功响应，测试中也出现过上游超时，不承诺持续可用性。详见 [Cloudflare 验证记录](cloudflare/VALIDATION.md)。
