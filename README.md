...
**直连限制：用户随后明确反馈，当前 `workers.dev` 地址关闭代理后连网页也打不开，启用代理后才正常。此前“访问正常”和“定位正常”的反馈不能作为国内直连验收通过的依据。正在测试免费备用入口，尚未完成免代理访问验收。**
...
2026-09-24：用户已确认正式页面在手机上能打开并正常随机抽取，启用地图后再次确认“定位正常”。正式云端的地点搜索、附近查店、逆地理编码及 SDK 坐标转换已通过真实接口验证，网页选点与门店展示通过交互验证；手机导航跳转尚未真机验收。此前开发电脑的 TLS 握手问题在本次验证中未复现，不代表所有网络始终可用。详见 [Cloudflare 验证记录](cloudflare/VALIDATION.md)。
